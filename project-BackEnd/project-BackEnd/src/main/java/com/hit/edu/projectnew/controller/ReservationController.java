...
import com.hit.edu.projectnew.service.*;
...
public class ReservationController {
    ...
    public Map<String, Object> reserveClassroom(@RequestBody Map<String, String> reservationInfo) {
        ...
        // 检查账号是否在学生表或教师表或管理员表中
        boolean isStudent = studentService.isStudent(reservations);
        boolean isTeacher = teacherService.isTeacher(reservations);
        boolean isAdmin = adminService.isAdmin(reservations);
        if (!isStudent && !isTeacher && !isAdmin) {
            response.put("success", false);
            response.put("message", "Account not found in user records.");
            return response;
        }

        List<reservation> reservationlist = reservationService.getReservationsByDateAndCIDAndTime(dateTime, CID, occuTime);
        if(reservationlist.size() != 0){
            for (reservation res :reservationlist){
                if(res.getOccuStatus() != 1){
                    response.put("success", false);
                    response.put("message", "classroom occupied!");
                    return response;
                }
            }
        }
...
//    @PostMapping
    public Map<String, Object> updateChecklist(@RequestBody Map<String, String> checklistInfo) {
        ...
        try {
            ...
            String dateString = checklistInfo.get("dateTime");
            LocalDate dateTime = LocalDate.parse(dateString);
...
//            审核通过
            if (checkStatus==1){
                checklistService.updateStatuses(checklist);
            }
            response.put("success", true);
...
//                reservationService.updateClassroomStatus(CID, dateTime, occuTime, status);
                // 然后删除对应的预约和审核记录
                reservation.setReservations("repair");
